...
export function registerReadonlyTools(server, context) {
  ...
  for (const toolName of ["genbio_h100_direct_status", "genbio_h100_mirror_status", "genbio_aizyme_h100_status", "genbio_aizyme_h100_prepare_status"]) {
    server.registerTool(toolName, { description: "Return local durable H100 status; remote reconciliation is added by the H100 phase.", inputSchema: { run_id: z.string().min(1).max(128).optional(), owner_handle: ownerSchema }, annotations: READ_ONLY }, async ({ run_id, owner_handle }) => { const runs = await createRunRegistry(context.config.runRegistryDir).list(owner_handle ?? "default"); const selected = run_id ? runs.filter((item) => item.runId === run_id) : runs.filter((item) => ["h100-direct", "aizyme"].includes(item.source)); return result({ ok: true, runs: selected }, `Returned ${selected.length} H100 records.`); });
  }
}
