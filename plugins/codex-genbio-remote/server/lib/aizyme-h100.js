...
const LOCAL_REMOTE_BUNDLE = join(import.meta.dirname, "..", "fixtures", "aizyme-h100");
...
async function loadLocalFiles() {
  ...
  for (const name of H100_FILES) {
    ...
    const path = join(LOCAL_REMOTE_BUNDLE, name);
...
export function createAizymeH100Tools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess, runRegistry }) {
  ...
  const stageTool = makeTool(
    ...
    async (_args, exec) => {
      ...
      const { files, bytes } = await loadLocalFiles();
      ...
      const inFlightH100 = state.aizymeH100InFlight instanceof Set ? state.aizymeH100InFlight : (state.aizymeH100InFlight = new Set());
      ...
      if (!userQuestions) throw new Error("material transfer requires MCP elicitation");
          ...
          const done = (async () => {
            ...
            try {
              ...
              // no-launch (conflict) from transport ambiguity (ssh failure).
               const initCmd = `set -u; stage_root=${shellQuote(H100_STAGE_ROOT)}; lock=${shellQuote(lockDir)}; if ! mkdir -p "$stage_root" 2>/dev/null; then printf 'STAGE_ROOT_ERROR=1\n'; exit 2; fi; if test -d "$lock"; then old_token=$(sed -n 's/^token=//p' "$lock/token.txt" 2>/dev/null | head -1); stale=0; reason=; case "$old_token" in (*[!a-f0-9]*|'') ;; (*) if test "\${#old_token}" -eq 32; then old_run="$stage_root/stage2-h100-$old_token"; if ! test -e "$old_run"; then stale=1; reason=absent_run_dir; elif test -f "$old_run/run_identity" && test -f "$old_run/exit_code" && test "$(sed -n 's/^token=//p' "$old_run/run_identity" 2>/dev/null | head -1)" = "$old_token"; then old_pid=$(sed -n 's/^pid=//p' "$old_run/run_identity" 2>/dev/null | head -1); old_exit=$(cat "$old_run/exit_code" 2>/dev/null || echo pending); if test "$old_exit" != pending && { test -z "$old_pid" || ! kill -0 "$old_pid" 2>/dev/null; }; then stale=1; reason=terminal_token_bound_run; fi; elif test ! -e "$old_run/run_identity" && test ! -e "$old_run/exit_code" && grep -Fq 'AIZH100_RUN_DIR must be set by the plugin launcher' "$old_run/stderr.log" 2>/dev/null; then lock_age=$(( $(date +%s) - $(stat -c %Y "$lock/token.txt" 2>/dev/null || echo 0) )); if test "$lock_age" -ge 300; then stale=1; reason=historical_env_i_failure; fi; fi; fi ;; esac; if test "$stale" -eq 1 && test "$(sed -n 's/^token=//p' "$lock/token.txt" 2>/dev/null | head -1)" = "$old_token"; then rm -f "$lock/token.txt"; if rmdir "$lock" 2>/dev/null; then printf 'STALE_LOCK_CLEARED=%s\n' "$reason"; else printf 'LOCK_CONFLICT=1\n'; exit 0; fi; else printf 'LOCK_CONFLICT=1\n'; exit 0; fi; fi; if ! mkdir "$lock" 2>/dev/null; then if test -d "$lock"; then printf 'LOCK_CONFLICT=1\n'; exit 0; fi; printf 'LOCK_ERROR_NO_LOCK=1\n'; exit 3; fi; if ! printf 'token=%s\nlocked_utc=%s\n' '${runToken}' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$lock/token.txt"; then rmdir "$lock" 2>/dev/null || true; printf 'LOCK_TOKEN_ERROR=1\n'; exit 4; fi; if ! mkdir -p ${shellQuote(remoteRunDir)} 2>/dev/null; then rm -f "$lock/token.txt"; rmdir "$lock" 2>/dev/null || true; printf 'RUN_DIR_ERROR_NO_LOCK=1\n'; exit 5; fi; printf 'LOCK_ACQUIRED=1\n'`;
  ...
  const statusTool = makeTool(
    ...
    async (args, exec) => {
      ...
      const run = state.runs.find((item) => item.runId === runId && item.operation === "aizyme-h100-stage2");
      if (!run) return { ok: true, status: { ...publicState(state), h100Status: { found: false, runId } } };
      ...
      const failureEvidence = section("FAILURE_EVIDENCE").slice(-12000);
      ...
      if (reconciled === "completed" || reconciled === "failed") {
        ...
        if (run.remoteLockDir) {
          try {
            const releaseCmd = `set -eu; lock=${shellQuote(run.remoteLockDir)}; lock_token=$(sed -n 's/^token=//p' "$lock/token.txt" 2>/dev/null | head -1); if test "$lock_token" = '${run.runToken}'; then rm -f "$lock/token.txt"; rmdir "$lock"; printf 'LOCK_RELEASED=1\n'; else printf 'LOCK_RELEASED=0\n'; fi; if test -d "$lock"; then printf 'LOCK_STILL_PRESENT=1\n'; exit 1; fi`;
