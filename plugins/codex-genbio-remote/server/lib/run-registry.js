...
const MAX_CROSS_SESSION_SESSIONS = 64;
const MAX_CROSS_SESSION_FILES = 64;
const MAX_CROSS_SESSION_BYTES = 4 * 1024 * 1024;
const SAFE_SESSION_RE = /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/u;
const SAFE_REGISTRY_RUN_ID_RE = /^rr-[A-Za-z0-9_-]{1,125}$/u;
...
export function createRunRegistry(rootDir) {
  ...
  function parseRecords(text) {
    ...
    const allowed = new Set(["schema", "updatedAt", "records"]);
    for (const key of Object.keys(parsed)) if (!allowed.has(key)) throw new Error(`run registry file has an unknown field and is rejected: ${key}`);
    const records = parsed.records.map((record) => normalizeRecord(record));
    const ids = new Set();
    for (const record of records) {
      if (ids.has(record.runId)) throw new Error(`run registry duplicate runId ${record.runId}; refusing to load (fail closed)`);
      ids.add(record.runId);
    }
    return records;
  }

  async function readRegistryFileNoFollow(file) {
    let handle;
    try {
      handle = await open(file, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
      const info = await handle.stat();
      if (!info.isFile()) throw new Error("run registry file is not a regular file; refusing to load (fail closed)");
      if (info.size > MAX_FILE_BYTES) throw new Error(`run registry file exceeds ${MAX_FILE_BYTES} bytes; refusing to load (fail closed)`);
      return await handle.readFile("utf8");
    } catch (error) {
      if (error?.code === "ENOENT") return null;
      if (error?.code === "ELOOP") throw new Error("run registry file is a symlink; refusing to follow (fail closed)");
      if (error?.message?.includes("refusing to load")) throw error;
  ...
  async function reconcileLoadedEntry(entry, records) {
  ...
  async function loadEntry(entry, { noFollow = false } = {}) {
    if (entry.records !== null) return entry.records;
    let text;
    if (noFollow) {
      text = await readRegistryFileNoFollow(entry.file);
      if (text === null) { entry.records = []; return entry.records; }
    } else {
      try {
    ...
    return reconcileLoadedEntry(entry, parseRecords(text));
  }
  ...
  /**
   * Find an active run by exact identity across immediate session-* folders.
   * This is local-only recovery: it never adopts a run or enumerates remote
   * paths. Loading uses normal restart reconciliation semantics, so an old
   * in-flight row may durably become reconciling. Bounds and malformed data
   * fail closed; a duplicate is reported as ambiguous, never guessed.
   */
  async function findActiveByIdentity({ source, project, operation }) {
    if (!SOURCES.has(source)) throw new Error(`run registry lookup source is invalid: ${source}`);
    if (typeof project !== "string" || !SAFE_NAME_RE.test(project)) throw new Error(`run registry lookup project is invalid: ${project}`);
    if (typeof operation !== "string" || !SAFE_NAME_RE.test(operation)) throw new Error(`run registry lookup operation is invalid: ${operation}`);
    let rootInfo;
    try { rootInfo = await lstat(rootDir); }
    catch (error) {
      if (error?.code === "ENOENT") return { found: false, outcome: "none" };
      throw new Error(`run registry root load failed: ${String(error?.message ?? error)}`);
    }
    if (!rootInfo.isDirectory()) throw new Error("run registry root is not a directory; refusing cross-session lookup");
    let entries;
    try { entries = await readdir(rootDir, { withFileTypes: true }); }
    catch (error) { throw new Error(`run registry session scan failed: ${String(error?.message ?? error)}`); }
    const sessionsToScan = entries.filter((entry) => entry.name.startsWith("session-")).sort((a, b) => a.name.localeCompare(b.name));
    if (sessionsToScan.length > MAX_CROSS_SESSION_SESSIONS) throw new Error(`run registry cross-session lookup exceeds ${MAX_CROSS_SESSION_SESSIONS} session directories; refusing to scan`);
    let filesRead = 0;
    let bytesRead = 0;
    let match = null;
    for (const dirent of sessionsToScan) {
      const sessionId = dirent.name;
      if (!SAFE_SESSION_RE.test(sessionId)) throw new Error(`unsafe run registry session id during cross-session lookup: ${sessionId}`);
      if (dirent.isSymbolicLink()) throw new Error(`run registry session directory is a symlink; refusing to follow: ${sessionId}`);
      if (!dirent.isDirectory()) throw new Error(`run registry session entry is not a directory: ${sessionId}`);
      const dir = join(rootDir, sessionId);
      const dirInfo = await lstat(dir);
      if (!dirInfo.isDirectory()) throw new Error(`run registry session directory is not a directory: ${sessionId}`);
      const file = join(dir, "runs.json");
      filesRead += 1;
      if (filesRead > MAX_CROSS_SESSION_FILES) throw new Error(`run registry cross-session lookup exceeds ${MAX_CROSS_SESSION_FILES} registry files; refusing to scan`);
      const text = await readRegistryFileNoFollow(file);
      if (text === null) continue;
      const textBytes = Buffer.byteLength(text, "utf8");
      if (bytesRead + textBytes > MAX_CROSS_SESSION_BYTES) throw new Error(`run registry cross-session lookup exceeds ${MAX_CROSS_SESSION_BYTES} bytes; refusing to scan`);
      bytesRead += textBytes;
      const entry = sessionEntry(sessionId);
      const records = await queue(entry, async () => reconcileLoadedEntry(entry, parseRecords(text)));
      for (const record of records) {
        if (!SAFE_REGISTRY_RUN_ID_RE.test(record.runId) || record.source !== source || record.project !== project || record.operation !== operation || !NON_TERMINAL_STATUSES.has(record.status)) continue;
        if (match !== null) { match = { ambiguous: true }; continue; }
        match = { sessionId, record: structuredClone(record) };
      }
    }
    if (match === null) return { found: false, outcome: "none" };
    if (match.ambiguous) return { found: false, outcome: "ambiguous" };
  ...
  return Object.freeze({ record, update, list, load, findActiveByIdentity, drop, rootDir, limits: Object.freeze({ maxRecords: MAX_RECORDS, maxNoteChars: MAX_NOTE_CHARS, maxFileBytes: MAX_FILE_BYTES, maxCrossSessionSessions: MAX_CROSS_SESSION_SESSIONS, maxCrossSessionFiles: MAX_CROSS_SESSION_FILES, maxCrossSessionBytes: MAX_CROSS_SESSION_BYTES }) });
}

export { SCHEMA as RUN_REGISTRY_SCHEMA, MAX_RECORDS, MAX_NOTE_CHARS, MAX_FILE_BYTES, MAX_CROSS_SESSION_SESSIONS, MAX_CROSS_SESSION_FILES, MAX_CROSS_SESSION_BYTES, SAFE_SESSION_RE, SAFE_REGISTRY_RUN_ID_RE, SOURCES as RUN_REGISTRY_SOURCES, TERMINAL_STATUSES as RUN_REGISTRY_TERMINAL_STATUSES, NON_TERMINAL_STATUSES as RUN_REGISTRY_NON_TERMINAL_STATUSES };
