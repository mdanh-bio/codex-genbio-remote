...
const TERMINAL_RUN_STATUSES = new Set(["completed", "failed", "killed", "cancelled", "evidence-incomplete"]);
const REVIEW_STATUSES = new Set(["failed", "killed", "cancelled", "evidence-incomplete"]);
...
function aggregateOne(summary, plans, runs) {
  ...
  for (const run of projectRuns) {
    ...
    else if (status === "failed" || status === "evidence-incomplete") run_counts.failed += 1;
