...
import { lstat, mkdir, mkdtemp, readFile, readdir, link, rename, rm, stat, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve, sep } from "node:path";
import { projectTarget } from "./project.js";
...
import { gpuCount, queueProbeBody, parseQueueGpuAllocation } from "./node-gpu-headroom.js";
import { withDirectoryLock } from "./atomic-store.js";
...
function validateEnvelope(state, requested, target = "HPC") {
  const envelope = state.envelope;
  if (!envelope) throw new Error("set the HPC session envelope before running a project operation");
  if (envelope.target !== target || !["HPC", "NHPC"].includes(target) || envelope.partition !== (target === "NHPC" ? "gpu" : "gpus")) throw new Error("project operations require a matching HPC/gpus or NHPC/gpu envelope");
...
function markAllocationTerminal(state, slurmJobId, stateText, target = "HPC") {
  let found = false;
  for (const entry of allocationsOf(state)) if ((entry.target ?? "HPC") === target && entry.slurmJobId === slurmJobId && ["nonterminal", "ambiguous", "cancel-requested"].includes(entry.status)) { entry.status = "terminal"; entry.terminalState = stateText ?? null; entry.terminalAt = Date.now(); found = true; }
  return found;
}
...
async function stageAndValidate({ manifest, exec, userQuestions, shell, runRemote, config, extraOwnPrefixes = [] }) {
  ...
  await askTransfer(userQuestions, exec, manifest.project, files, bytes, target);
  ...
  const inspect = await runRemoteWithRetry(runRemote, target, strictRemote(target, `set -eu; if test -e ${root}; then cd -- ${root}; find . -type f -printf '%P\\n' | sort; else printf '__NO_ROOT__\\n'; fi`), exec, 30000);
  ...
  const remoteInit = await runRemote(target, strictRemote(target, `set -eu; mkdir -p ${dirs}`), exec, 30000);
  ...
  const rcloneRemote = resolveRcloneRemote(config, target);
  ...
  const validation = await runRemote(target, strictRemote(target, segments.join("; ")), exec, 60000);
...
export async function probeNodeHeadroom({ exec, runRemote, cpus, gpus, node, target = "HPC", policy = null }) {
  ...
  const body = `set -eu; line=$(scontrol show node ${node} -o); printf 'NODE_PROBE=%s\\n' "$line"`;
  ...
  if (lines.length !== 1) throw new Error(`${node} pre-submit probe: duplicate or unexpected node evidence`);
  ...
  const integer = (text) => /^[0-9]+$/u.test(text ?? "") && Number.isSafeInteger(Number(text)) ? Number(text) : NaN;
  const nodeName = field("NodeName"), stateText = field("State"), cores = integer(field("CPUTot")), allocCores = integer(field("CPUAlloc"));
  ...
  if (!USABLE_NODE_STATES.has(baseState) || /[+~*#]/u.test(stateText)) throw new Error(`${node} pre-submit probe: node state "${stateText}" is not usable (idle/alloc/mix); failing closed before sbatch`);
  const configuredGpus = gpuCount(field("CfgTRES")) ?? gpuCount(field("Gres"));
  const allocatedTresGpus = gpuCount(field("AllocTRES"));
  let allocatedGpus = allocatedTresGpus;
  const effectiveCores = field("CPUEfctv") === null ? cores : integer(field("CPUEfctv"));
  const freeCpus = Math.min(cores, effectiveCores) - allocCores;
  ...
  if (![cores, allocCores, effectiveCores].every(Number.isSafeInteger) || cores <= 0 || freeCpus < 0) throw new Error(`${node} pre-submit probe: missing or non-numeric CPU/GPU resources`);
  let gpuSource = "AllocTRES";
  if (gpusReq > 0 && allocatedGpus === null && baseState !== "idle") {
    const queue = await runRemote(target, strictRemote(target, queueProbeBody(node)), exec, 30000);
    if (queue.exitCode !== 0 || queue.timedOut || queue.signal) throw new Error(`${node} pre-submit probe: allocated-GPU queue query failed; failing closed before sbatch`);
    const evidence = parseQueueGpuAllocation(String(queue.stdout), node, value);
    allocatedGpus = evidence.allocatedGpus;
    const configuredCap = policy?.targets?.[target]?.allowlist?.[node]?.caps?.max_concurrent_gpu_jobs;
    if (Number.isSafeInteger(configuredCap) && configuredCap > 0 && evidence.gpuJobs >= configuredCap) throw new Error(`${node} pre-submit probe: concurrent GPU job policy cap reached`);
    gpuSource = "stable-single-node-queue";
  }
  ...
  return { ok: true, freeCpus, freeGpus: gpusReq > 0 ? freeGpus : null, gpuSource: gpusReq > 0 ? gpuSource : null, note: `${node} state=${stateText} free_cpus=${freeCpus} free_gpus=${gpusReq > 0 ? freeGpus : "not-requested"}` };
}
...
async function verifyPackage({ manifest, exec, runRemote }) {
  ...
  const root = shellQuote(manifest.remoteRoot);
  const rels = files.map((item) => shellQuote(item.rel)).join(" ");
  const result = await runRemote(target, strictRemote(target, `set -eu; cd -- ${root}; test -s PREPARED_SHA256.txt; sha256sum -c --quiet PREPARED_SHA256.txt; sha256sum ${rels}`), exec, 60000);
...
async function stageRecipeWrapper({ manifest, wrapperBytes, operation, manifestSha, exec, userQuestions, shell, runRemote, config }) {
  ...
  try {
    const mkd = await runRemote(target, strictRemote(target, `set -eu; mkdir -p ${shellQuote(`${manifest.remoteRoot}/genbio-recipes`)}`), exec, 30000);
    ...
    const check = await runRemoteWithRetry(runRemote, target, strictRemote(target, `set -eu; cd -- ${root}; sha256sum ${shellQuote(wrapperRel)}`), exec, 30000);
    ...
    const syntax = await runRemoteWithRetry(runRemote, target, strictRemote(target, `set -eu; cd -- ${root}; env -i PATH=/usr/bin:/bin BASH_ENV=/dev/null /bin/bash --noprofile --norc -n -- ${shellQuote(wrapperRel)}`), exec, 30000);
...
export async function reconcileSubmission({ exec, runRemote, jobName, target = "HPC" }) {
...
async function submitJob({ manifest, operation, policy, state, exec, runRemote, intent, allocation, templateSha, templateRel, templateBytes, beforeDispatch = null }) {
  ...
  const revalidated = validatePinnedSbatch(templateBytes.toString("utf8"), jobSpec, { policy, envelope: state.envelope, target });
  ...
  if (pending) {
    const resolved = await reconcileSubmission({ exec, runRemote, target, jobName: pending.uniqueJobName });
    if (resolved !== null) {
  ...
  try {
    await probeNodeHeadroom({ exec, runRemote, cpus: jobSpec.cpus, gpus: jobSpec.gpus, node: revalidated.node, target, policy });
  } catch (error) {
  ...
  const submit = await runRemote(target, strictRemote(target, `set -eu; cd -- ${root}; job_id=$(sbatch --parsable --job-name=${shellQuote(intent.uniqueJobName)} ${shellQuote(submitTemplate)}); test -n "$job_id"; printf 'JOB_ID=%s\\nRUN_DIR=%s\\n' "$job_id" ${root}`), exec, 60000);
  if (submit.exitCode !== 0) {
    if (isTransportFailure(submit)) {
      ...
      const resolved = await reconcileSubmission({ exec, runRemote, target, jobName: intent.uniqueJobName });
      if (resolved !== null) {
        intent.status = "submitted"; intent.slurmJobId = resolved; intent.resolvedAt = Date.now();
...
function findWorkloadRun(state, project, jobId, target = null) {
  return [...(state.runs ?? [])].reverse().find((run) => (target === null || run.target === target) && run.slurmJobId === jobId && String(run.operation).startsWith(`project-${project}-`) && !String(run.operation).includes("status-")) ?? null;
}

function findOwnedOperationRun(state, project, operation, jobId, target = null) {
  return [...(state.runs ?? [])].reverse().find((run) => (target === null || run.target === target) && run.slurmJobId === jobId && run.operation === `project-${project}-${operation}`) ?? null;
}
...
async function cancelOwnedJob({ state, project, operation, jobId, target: requestedTarget = null, exec, userQuestions, runRemote, expectedJobName = null, beforeCancel = null }) {
  const target = projectTarget({ target: requestedTarget ?? state.envelope?.target });
  ...
  if (!owned || !findOwnedOperationRun(state, project, operation, exactJobId, target)) throw new Error(`Slurm job ${exactJobId} is not an active session-owned job for ${project}/${operation}`);
  if (expectedJobName) {
    const identity = await runRemote(target, strictRemote(target, `set -eu; job=${shellQuote(exactJobId)}; expected=${shellQuote(expectedJobName)}; table=$(sacct -X -n -u "$USER" -j "$job" --format=JobIDRaw,JobName -P 2>/dev/null || true); matches=$(printf '%s\\n' "$table" | awk -F'|' -v id="$job" -v name="$expected" '$1 == id && $2 == name { n++ } END { print n+0 }'); test "$matches" = 1`), exec, 30000);
  ...
  const result = await runRemote(target, strictRemote(target, `set -eu; scancel ${shellQuote(exactJobId)}`), exec, 30000);
...
async function statusJob({ manifest, jobId, state, exec, runRemote }) {
  ...
  const run = findWorkloadRun(state, manifest.project, jobId, target);
  ...
  const intent = [...submissionsOf(state)].reverse().find((entry) => (entry.target ?? target) === target && entry.project === manifest.project && entry.operation === operation && entry.slurmJobId === jobId);
  ...
  const result = await runRemote(target, strictRemote(target, body), exec, 60000);
  ...
  if (TERMINAL_SCHEDULER_STATES.has(parsed.state)) markAllocationTerminal(state, jobId, parsed.state, target);
...
async function assertNoSymlinkAncestors(root, destination) {
  const absoluteRoot = resolve(root), absoluteDestination = resolve(destination);
  if (absoluteDestination !== absoluteRoot && !absoluteDestination.startsWith(absoluteRoot + sep)) throw new Error("fetch destination escapes local root");
  const relative = absoluteDestination.slice(absoluteRoot.length).split(sep).filter(Boolean);
  let current = absoluteRoot;
  if ((await lstat(current)).isSymbolicLink()) throw new Error("fetch local root is a symbolic link");
...
async function fetchArtifacts(args) {
  const { manifest } = args;
  // One queue per local destination, including distinct runs configured to collide.
  const key = manifest.localRoot && manifest.fetch ? resolve(manifest.localRoot, manifest.fetch.dest) : `${manifest.project}:${manifest.remoteRoot}`;
  const previous = fetchQueues.get(key) ?? Promise.resolve();
  const next = previous.catch(() => {}).then(async () => {
    if (!manifest.localRoot || !manifest.fetch) return fetchArtifactsUnlocked(args);
    try { await lstat(manifest.localRoot); } catch (error) { if (error.code === "ENOENT") return fetchArtifactsUnlocked(args); throw error; }
    await assertNoSymlinkAncestors(manifest.localRoot, key);
    const lockRoot = join(manifest.localRoot, `.genbio-fetch-lock-${sha256(key).slice(0, 24)}`);
    await assertNoSymlinkAncestors(manifest.localRoot, lockRoot);
    return withDirectoryLock(lockRoot, { pid: process.pid, target: projectTarget(manifest), remoteRoot: manifest.remoteRoot }, () => fetchArtifactsUnlocked(args));
  });
  fetchQueues.set(key, next);
  try { return await next; }
  finally { if (fetchQueues.get(key) === next) fetchQueues.delete(key); }
}

async function fetchArtifactsUnlocked({ manifest, requested, exec, userQuestions, shell, runRemote, config }) {
  const target = projectTarget(manifest);
  ...
  const discovery = await runRemoteWithRetry(runRemote, target, strictRemote(target, `set -eu; cd -- ${root}; for f in ${forList}; do if test -f "$f" && ! test -L "$f"; then printf 'OK|%s|%s|%s\\n' "$f" "$(stat -c %s "$f")" "$(sha256sum "$f" | cut -d' ' -f1)"; else printf 'MISSING|%s\\n' "$f"; fi; done`), exec, 30000);
  ...
  for (const line of String(discovery.stdout).split(/\r?\n/u)) {
    ...
    if (parts.length !== 4 || parts[0] !== "OK") throw new Error("invalid or missing remote artifact discovery evidence");
    const [ , rel, sizeText, sha] = parts;
    if (!rels.includes(rel) || discovered.has(rel) || !/^(0|[1-9][0-9]*)$/u.test(sizeText) || !Number.isSafeInteger(Number(sizeText)) || !/^[a-f0-9]{64}$/u.test(sha)) throw new Error("invalid or duplicate remote artifact discovery evidence");
    discovered.set(rel, { size: Number(sizeText), sha });
  }
  ...
  await askFetch(userQuestions, exec, manifest.project, rels, totalBytes, destDir, target);
  ...
  try {
    ...
    const verification = await runRemoteWithRetry(runRemote, target, strictRemote(target, `set -eu; cd -- ${root}; for f in ${forList}; do test -f "$f" && ! test -L "$f"; printf 'OK|%s|%s|%s\\n' "$f" "$(stat -c %s "$f")" "$(sha256sum "$f" | cut -d' ' -f1)"; done`), exec, 30000);
    ...
    receiptName = `FETCH_RECEIPT_${stamp}_${randomBytes(8).toString("hex")}.json`;
    ...
    try { const info = await lstat(destDir); if (!info.isDirectory() || info.isSymbolicLink()) throw new Error(`${manifest.project}: fetch destination is not a regular directory`); }
    catch (error) { if (error?.code === "ENOENT") destinationExists = false; else throw error; }
    if (!destinationExists) {
      await rename(stageDir, destDir);
    } else {
      const verified = new Map();
      const names = await readdir(destDir);
      const receipts = names.filter((name) => /^FETCH_RECEIPT_.*\.json$/u.test(name));
      if (receipts.length === 0) throw new Error("existing fetch destination has no verified receipt");
      for (const name of receipts) {
        const receiptPath = join(destDir, name);
        const info = await lstat(receiptPath);
        if (!info.isFile() || info.isSymbolicLink() || info.size > 1024 * 1024) throw new Error("invalid fetch receipt file");
        const receipt = JSON.parse(await readFile(receiptPath, "utf8"));
        if (receipt.receipt_schema !== "genbio-project-fetch/1" || receipt.project !== manifest.project || receipt.source_root !== manifest.remoteRoot || receipt.rclone_remote !== rcloneRemote || !Array.isArray(receipt.files) || receipt.files.length === 0) throw new Error("corrupt or mismatched fetch receipt");
        for (const item of receipt.files) {
          if (!allowlist.has(item.rel) || !Number.isSafeInteger(item.size) || item.size < 0 || !/^[a-f0-9]{64}$/u.test(item.sha256)) throw new Error("corrupt fetch receipt entry");
          const prior = verified.get(item.rel);
          if (prior && (prior.size !== item.size || prior.sha256 !== item.sha256)) throw new Error("conflicting fetch receipts");
          const path = join(destDir, item.rel);
          await assertNoSymlinkAncestors(manifest.localRoot, path);
          const file = await lstat(path);
          if (!file.isFile() || file.isSymbolicLink() || file.size !== item.size || sha256(await readFile(path)) !== item.sha256) throw new Error("fetch receipt content mismatch");
          verified.set(item.rel, item);
        }
      }
      // Validate the complete merge before publishing any new file.
      for (const item of transferred) {
        const prior = verified.get(item.rel);
        if (prior && (prior.size !== item.size || prior.sha256 !== item.sha256)) throw new Error(`${item.rel}: destination content conflicts with verified receipt`);
        const path = join(destDir, item.rel);
        await assertNoSymlinkAncestors(manifest.localRoot, path);
        try { await lstat(path); if (!prior) throw new Error(`${item.rel}: existing file has no receipt`); }
        catch (error) { if (error?.code !== "ENOENT") throw error; }
      }
      for (const item of transferred) {
        const source = join(stageDir, item.rel);
        const destination = join(destDir, item.rel);
        await assertNoSymlinkAncestors(destDir, destination);
        try {
          const existing = await lstat(destination);
          if (existing.isSymbolicLink() || !existing.isFile()) throw new Error(`${item.rel}: destination conflicts with a non-regular file`);
          if (existing.size !== item.size || sha256(await readFile(destination)) !== item.sha256) throw new Error(`${item.rel}: destination content conflicts with verified receipt`);
      ...
      await link(join(stageDir, receiptName), join(destDir, receiptName));
...
function makeRun(project, operation, requested, state, requestedTarget = null, identity = {}) {
  const target = projectTarget({ target: requestedTarget ?? state.envelope?.target });
  return { runId: `${target}-project-${project}-${operation}-${Date.now()}`, target, operation: `project-${project}-${operation}`, status: "running", helperStatus: "running", workloadStatus: requested.cpus === 0 && requested.gpus === 0 ? "not_applicable" : "unsubmitted", startedAt: Date.now(), finishedAt: null, stdout: "", stderr: "", error: null, slurmJobId: null, slurmStatus: null, slurmExitCode: null, slurmElapsed: null, workloadEvidence: "unobserved", resources: { cpus: requested.cpus, gpus: requested.gpus, memGb: null, concurrency: requested.concurrency }, policyHash: state.policy.hash, node: identity.node ?? state.envelope?.node ?? null, partition: identity.partition ?? state.envelope?.partition ?? null, envelope: state.envelope ? JSON.parse(JSON.stringify(state.envelope)) : null, memory: { status: "not-finalized", error: null, openVikingSessionId: null, traceId: null } };
}

function startTrackedJob({ jobs, exec, state, project, operation, resources, target = null, identity = {}, runBody, label, logMaxBytes }) {
  if (!jobs) throw new Error("background job registry is unavailable");
  const maxBytes = Number(logMaxBytes ?? 65536);
  const run = makeRun(project, operation, resources, state, target, identity);
  run.runId = run.target + "-project-" + randomBytes(16).toString("hex");
  run.jobId = jobs.start({ kind: "genbio-" + run.target + "-project", label, owner: exec.agent, run: () => {
